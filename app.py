...
def model_selection(num: str):
    model = ''
    if num == '1':
        model = 'best_model_grid_320_server.h5'
    elif num == '2':
        model = 'best_model_grid_480_server.h5'
    elif num == '3':
        model = 'best_model_grid_640_server.h5'
    elif num == '4':
        model = 'best_model_grid_1280_server.h5'
    elif num == '5':
        model = 'best_model_grid_2560_server.h5'
    else:
        raise ValueError("Model id should be the above 5 model")
    return model
...
def predict():
    ...
    scaler_name = model_name.split('.')[0] + '.joblib'
    print(scaler_name)
    scaler = joblib.load(os.path.join(os.getcwd(),scaler_name))
    ...
    for seq in sequence_list:
        ...
        tuple_sequence = tuple(format_seq)
        peptide_sequence_list.append(tuple_sequence)  # build a summarize list variable including all the sequence information
    # select a embedding function
    if model_name == 'best_model_grid_320_server.h5':
        model, alphabet = esm.pretrained.esm2_t6_8M_UR50D()
        embeddings_results = esm_embeddings_320(model, alphabet, peptide_sequence_list)  # conduct the embedding
    if model_name == 'best_model_grid_480_server.h5':
        model, alphabet = esm.pretrained.esm2_t12_35M_UR50D()
        embeddings_results = esm_embeddings_480(model, alphabet, peptide_sequence_list)  # conduct the embedding
    if model_name == 'best_model_grid_640_server.h5':
        model, alphabet = esm.pretrained.esm2_t30_150M_UR50D()
        model, alphabet = esm.pretrained.esm2_t30_150M_UR50D()
        embeddings_results = esm_embeddings_640(model, alphabet, peptide_sequence_list)  # conduct the embedding
    if model_name == 'best_model_grid_1280_server.h5':
        model, alphabet = esm.pretrained.esm2_t33_650M_UR50D()
        embeddings_results = esm_embeddings_1280(model, alphabet, peptide_sequence_list)  # conduct the embedding
    if model_name == 'best_model_grid_2560_server.h5':
        model, alphabet = esm.pretrained.esm2_t36_3B_UR50D()
        embeddings_results = esm_embeddings_2560(model, alphabet, peptide_sequence_list)  # conduct the embeddin
    embeddings_results
    normalized_embeddings_results = scaler.transform(embeddings_results)  # normalized the embeddings
    # prediction
    predicted_protability = model_CNN.predict(normalized_embeddings_results, batch_size=1)
